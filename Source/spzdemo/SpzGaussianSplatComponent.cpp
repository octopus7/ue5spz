...
namespace
{
	...
	void ComputeEllipseAxes(
		const FSpzSplatRenderPoint& Point,
	...
	class FSpzGaussianSplatSceneProxy final : public FPrimitiveSceneProxy
	{
	public:
		explicit FSpzGaussianSplatSceneProxy(const USpzGaussianSplatComponent* Component)
			...
			FMeshElementCollector& Collector) const override
		{
			...
			for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
			{
				...
				{
					const FSpzSplatRenderPoint& Point = RenderPoints[SortedSplat.PointIndex];
...
USpzGaussianSplatComponent::USpzGaussianSplatComponent()
{
...
void USpzGaussianSplatComponent::SetRenderPoints(TArray<FSpzSplatRenderPoint>&& InRenderPoints, const FVector& InLocalBoundsOrigin, const FVector& InLocalBoundsExtent)
{
...
void USpzGaussianSplatComponent::ClearRenderPoints()
{
	RenderPoints.Reset();
...
FPrimitiveSceneProxy* USpzGaussianSplatComponent::CreateSceneProxy()
{
	...
	return new FSpzGaussianSplatSceneProxy(this);
}

FBoxSphereBounds USpzGaussianSplatComponent::CalcBounds(const FTransform& LocalToWorld) const
{
...
int32 USpzGaussianSplatComponent::GetNumMaterials() const
{
	return 1;
}
